...
#         current_time = time.time()
#         if current_time - self.last_time > self.update_interval:
#             np_arr = np.frombuffer(cv_image.data, np.uint8) #압축해제
#             image_np = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
#             output = self.pose_estimation(image_np, self.aruco_dict_type, self.k, self.d)
#             cv2.imshow('Estimated Pose', output)
#             cv2.waitKey(1)
#             self.last_time = current_time # 마지막 업데이트 시간 갱신
...
#         self.subscriber_img = self.create_subscription(CompressedImage, 'image_raw/compressed', self.image_callback, qos_profile_sensor_data)
#         self.publisher_cmd_vel = self.create_publisher(Twist, '/base_controller/cmd_vel', 10)
#         self.publisher_debug_image_tracker = self.create_publisher(CompressedImage, '/image_raw/compressed_tracker', 10)
#         self.subscriber_vision_detection_result = self.create_subscription(CmdAndPoseVel, '/detection_result', self.detection_result_callback, 10)
#         self.i = 0
#         self.twist = Twist()
...
#         self.publisher_debug_image_tracker.publish(self.cv_bridge.cv2_to_imgmsg(cv_image, encoding = data.encoding))
...
#         for detection in data.detection_result:
...
#                         center_x, center_y = np.mean(first_marker, axis=0)
#                         x, y = frame.shape[:2]

#                         # 이미지 중심과 ArUco 마커 중심 사이의 오차 계산
#                         error_x = center_x -x / 2
#                         error_y = center_y -y / 2
...
# #     #minibot_aruco 쳐서 bashrc 활성화 시키기
# #     #source ~/venv/aruco/bin/activate   로 (aruco) 가상환경 활성화 시켜준다.
# #     #python pose_estimation_test2_pinkbot.py --K_Matrix calibration_matrix.npy --D_Coeff distortion_coefficients.npy --type DICT_5X5_100
...
# class PoseEstimationNode(Node):
#     def __init__(self):
#         super().__init__('arucominibot')
        
                                                       
#     def pose_estimation(self, frame, aruco_dict_type, matrix_coefficients, distortion_coefficients):
#         super().__init__('aruco_minibot')
#         gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#         cv2.aruco_dict = cv2.aruco.Dictionary_get(aruco_dict_type)
#         parameters = cv2.aruco.DetectorParameters_create()
...
#     def Centerfollow(pose_estimation,text_size, text_x, text_y, text_z):
#         # 프레임의 상단 중앙에 현재 시간을 표시합니다
#         self.text_size = cv2.getTextSize(self.time_string, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)[0]
#         self.text_x = (frame.shape[1] - text_size[0]) // 2  # 가로 중앙 정렬
#         self.text_y = text_size[1] + 10  # 상단 여백을 10 픽셀로 설정
#         self.text_z = (frame.shape[1] - text_size[0]) // 8
#         cv2.putText(frame, time_string, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 0), 2)
#         cv2.putText(frame, "M-1",(text_z, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 0), 2) # 좌측 상단 끝에 로봇 번호 추가
...
# def main():
#     ap = argparse.ArgumentParser()
#     ap.add_argument("-k", "--K_Matrix", required=True, help="/home/kang/minibot_aruco/src/my_minibot_aruco_package/calibration_matrix.npy")
#     ap.add_argument("-d", "--D_Coeff", required=True, help="/home/kang/minibot_aruco/src/my_minibot_aruco_package/distortion_coefficients.npy")
#     ap.add_argument("-t", "--type", type=str, default="DICT_ARUCO_ORIGINAL", help="Type of ArUCo tag to detect")
#     args = vars(ap.parse_args())
